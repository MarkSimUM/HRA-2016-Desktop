...
uses
  ...
  Arrow, ButtonPanel, Buttons, DataModuleMain, db, process, Ora, OraSmart,
  globals, VirtualTable, datamoduleproject;
...
begin
 ...
 // :PDELETE_FLG, :VRETURNCODE, :VSEQUENCENUM, :VUPDATEINSERT);
   ...
   if DBLookup_ProductVersion.KeyValue < 0 then
    begin
     ShowMessage('Enter a Product Version');
     exit ;
    end;

if  MessageDlg ('Question', 'Save new project for account '+DM_MAIN.dataAccountACCOUNTNAME.asString, mtConfirmation,
   ...
   //  ParamByName('PPROJECTGROUP_ID').value := ;
    ...
    // ParamByName('PPROJECT_END_DATE').value := ;
     ...
     execute;
         ...
         ShowMessage(' Create new Project with PROJECT_ID: '+ParamByName('VSEQUENCENUM').AsString );
         Close ;
